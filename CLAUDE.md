...
Most `WIDGET_DEFS[type].fetch()` are stubbed and return mock data with light randomness. **Weather** (`apps/web/src/data/weather.ts` → OpenWeather, browser-direct), **storeRating** (`apps/web/src/data/storeRating.ts` → `/api/store-rating` → `apps/proxy` → iTunes Lookup against the JP storefront for headline numbers + App Store Connect Customer Reviews API for trend / breakdown / delta), and **feedback** (`apps/web/src/data/feedback.ts` → `/api/feedback` → `apps/proxy` → GitHub Issues API against the private `TrainLCD/Issues` repo, filtered by the `🙏 Feedback` label) are connected to real sources. `feedback` items have no rating equivalent so `stars: 0` is returned and the widget hides the star row when stars is 0; `unread` is currently `items.length` until #3 wires localStorage-based last-seen. The proxy fetches a single 100-item page from GitHub and forwards the Link `rel="next"` presence as `hasMore`; the widget shows `100+` / `99+` instead of raw counts when truncated. The remaining two (`performance`, `tasks`) are still mocked and tracked by Issues #4–#5.
...
`apps/web/src/state/toolConnections.ts` is a `useSyncExternalStore`-backed module keyed by MCP tool name. Keys today: `openWeather`, `appStoreConnect`, `github`. `ToolsBadge` derives its count and per-tool dot colors from this store, and real data fetchers in `apps/web/src/data/` are responsible for calling `setToolConnected(name, true|false)` on success / failure. The store-rating fetcher reads `sources.appStore` and the feedback fetcher reads `sources.github` from their proxy responses and toggle the matching key. Add a new key here only when an actual connection lands.
...
- **Local secrets in root `.env.local`.** Both the browser-side keys (`VITE_OPENWEATHER_API_KEY`) and the proxy-only keys (`APP_STORE_CONNECT_*`, `GITHUB_TOKEN`) live in the repo root's `.env.local`. The proxy reads it via Node's `--env-file`; Vite reads it via `envDir: '../../'`. Type any new client-side key in `apps/web/src/vite-env.d.ts`. Missing key → the widget shows skeleton and the relevant tool stays disconnected; that's the intended fallback, not a bug.
